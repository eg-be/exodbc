...
void printHelp()
{
	...
	WRITE_STDOUT(u8"Usage: exOdbcGTest [OPTION]... [DATABASE]");
	WRITE_STDOUT(u8"Run unit-tests against DATABASE or read configuration from TestSettings.xml.");
	WRITE_STDOUT(u8"If DATABASE is not specified, all settings will be read from the file");
	WRITE_STDOUT(u8"TestSettings.xml, located in either the directory of the app, or in a");
	WRITE_STDOUT(u8"subdirectory named 'exOdbcGTest'. If the file is not found in the directory");
	WRITE_STDOUT(u8"of the app, all parent directories are searched the same way.");
	WRITE_STDOUT("");
	WRITE_STDOUT("If DATABASE is specified, you can either supply a connection-string or a DSN-entry");
	WRITE_STDOUT("to indicate the connection info of the test database:");
	WRITE_STDOUT(" To connect using a configured DSN entry use:");
	WRITE_STDOUT("");
	WRITE_STDOUT("  DSN=dsn;user;pass (to connect to a database with UPPER-case names (tables/columns))");
	WRITE_STDOUT("  dsn=dsn;user;pass (to connect to a database with lower-case names (tables/columns))");
	WRITE_STDOUT("");
	WRITE_STDOUT(" or to connect using a connection-string use:");
	WRITE_STDOUT("");
	WRITE_STDOUT("  CS=connectionString (to connect to a database with UPPER-case names (tables/columns))");
	WRITE_STDOUT("  cs=connectionString (to connect to a database with lower-case names (tables/columns))");
	WRITE_STDOUT("");
	WRITE_STDOUT("Note that you must frame the 'cs=connectionString' with \" if your connection");
	WRITE_STDOUT("string contains white spaces.");
	WRITE_STDOUT("");
	WRITE_STDOUT("OPTION can be:");
	WRITE_STDOUT(" --createDb       Run the scripts to create the test database before running the tests.");
	WRITE_STDOUT("                    The TestDbCreator will connect to the database and try to detect");
	WRITE_STDOUT("                    the database system. If there is a subdirectory matching the");
	WRITE_STDOUT("                    database system name in 'CreateScripts' directory, all scripts");
	WRITE_STDOUT("                    inside that directory are run.");
	WRITE_STDOUT(" --logLevelX      Set the log level, where X must be either");
	WRITE_STDOUT("                    E, W, I or D for Error, Warning, Info or Debug.");
	WRITE_STDOUT(" --logFile        Log additionally to a file 'exODbcGTest.log' in the current directory");
	WRITE_STDOUT(" --help           To show this help text.");
	WRITE_STDOUT("");
	WRITE_STDOUT("Examples:");
	WRITE_STDOUT(" exodbcGTest --createDb --logLevelW DSN=db2;uid;pass");
	WRITE_STDOUT("  to run the tests against a configured DSN entry named 'db2', using uppercase");
	WRITE_STDOUT("  names and log level Waring. Before the tests are run, the scripts to create");
	WRITE_STDOUT("  the test database are run.");
	WRITE_STDOUT(" exodbcGTest CS=\"Driver={IBM DB2 ODBC DRIVER};Database=EXODBC;Hostname=192.168.56.20;Port=50000;Protocol=TCPIP;Uid=db2ex;Pwd=extest;\"");
	WRITE_STDOUT("  to run the tests using a connection string, against a DB which uses uppercase.");
}
