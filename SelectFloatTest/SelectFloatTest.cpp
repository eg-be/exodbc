...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	SQLINTEGER cb1 = 0;
	SQLINTEGER cb2 = 0;
	SQLDOUBLE doubleVal = 0;
	SQLREAL floatVal = 0;
	ret = SQLBindCol(hstmt, 1, SQL_C_DOUBLE, &doubleVal, sizeof(doubleVal), &cb1);
	if (!SQL_SUCCEEDED(ret))
	{
		printErrorsAndAbort(SQL_HANDLE_STMT, hstmt);
	}
	ret = SQLBindCol(hstmt, 2, SQL_C_FLOAT, &floatVal, sizeof(floatVal), &cb2);
	if (!SQL_SUCCEEDED(ret))
	{
		printErrorsAndAbort(SQL_HANDLE_STMT, hstmt);
	}
	...
	// select value
	std::wstring sqlstmt = L"SELECT tdouble, tfloat FROM floattypes WHERE idfloattypes = 3";
